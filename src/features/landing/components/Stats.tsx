                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 items-center justify-items-center">
                    {STATS.map((stat, idx) => (
                        <div key={idx} className="flex flex-col items-center lg:items-start space-y-2 group">
                            <div className="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter group-hover:text-gold transition-colors duration-500">
                                {stat.value}
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 h-[1px] bg-gold/20"></div>
                                <div className="text-[0.6rem] font-black text-slate-400 uppercase tracking-[0.2em]">
                                    {stat.label} <span className="text-slate-300">{stat.suffix}</span>
                                </div>
                            </div>
                        </div>
                    ))}
